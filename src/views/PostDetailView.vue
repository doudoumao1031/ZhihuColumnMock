...
export default defineComponent({
    ...
    setup () {
        ...
        onMounted(() => {
            store.dispatch('fetchPosts', currentId, route.params)
            // console.log('user', store.getters.getCurrentPost())
        })
        ...
        const showEditArea = computed(() => {
            const { isLogin, columnId } = store.state.user

            if (currentPost.value && currentPost.value.author && isLogin) {
                // const postAuthor = currentPost.value.author as UserProps
                // return postAuthor._id === _id
                const postAuthor = currentPost.value.author as UserProps // 简化了数据结构硬写的 5f3e86d62c56ee13bb830961
                // console.log('user', postAuthor === columnId)
                return postAuthor._id === columnId
            } else {
                return false
            }
        })
